...
function equations_level_3 (max) {
    ...
    if (rng === 1) { // A+B(C+DX) = EX + F, solve for C, x = coeff_1, E = coeff_3*coeff_5, F = coeff_6*coeff_3, A = coeff_2*coeff_3, B = coeff_3, D = coeff_4
        ...
        answer = coeff_1;
        question = `${coeff_2*coeff_3} + ${coeff_3}( ${coeff_5*coeff_1 +coeff_6 - coeff_2 - coeff_4*coeff_1} + ${coeff_4}X) = ${coeff_5*coeff_3}X + ${coeff_6*coeff_3}`;
    } else if (rng === 2) { // A+BX = C(D+EX), solve for D, A = coeff_2*coeff_4, B = coeff_3*coeff_4, C = coeff_4, E = coeff_5
        infinite = infinite_check(3, rng, coeff_1, coeff_2, coeff_3, coeff_4, coeff_5, coeff_6, coeff_7, coeff_8);
        answer = coeff_1;
        question = `${coeff_2*coeff_4} + ${coeff_3*coeff_4}X = ${coeff_4}(${coeff_2+coeff_3*coeff_1 - coeff_5*coeff_1} + ${coeff_5}X)`;
        ...
        answer = coeff_1;
        question = `${coeff_2*coeff_3} + ${coeff_3}(${coeff_5-coeff_2 + coeff_6*(coeff_7 + coeff_8*coeff_1) - coeff_4*coeff_1} + ${coeff_4}X) = ${coeff_5*coeff_3} + ${coeff_6*coeff_3}(${coeff_7*coeff_3} + ${coeff_8*coeff_3}X)`;
    }
