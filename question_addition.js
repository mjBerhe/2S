function addition(questions, max, min, terms, decimals) {
    ...
    for (let j = 0; j < questions; j++) {
        ...
        if(terms == 2) {
            ...
            solution_add.push(first_v[j] + second_v[j]);
            string_v[j] = `${first_v[j]} + ${second_v[j]} ?`; }

        if(terms == 3) {

            solution_add.push(first_v[j] + second_v[j] + third_v[j]);
            string_v[j] = `${first_v[j]} + ${second_v[j]} + ${third_v[j]} ?`; }

        if(terms == 4) {

            solution_add.push(first_v[j] + second_v[j] + third_v[j] + fourth_v[j]);
            string_v[j] = `${first_v[j]} + ${second_v[j]} + ${third_v[j]} + ${fourth_v[j]}?`; }

        if(terms == 5) {

            solution_add.push(first_v[j] + second_v[j] + third_v[j] + fourth_v[j] + fifth_v[j]);
